...
export async function getDoctors() {
    try {
        ...
        if (response.ok) {
            // The API might wrap the array in an object { doctors: [...] } or just return the array
...
export async function deleteDoctor(id, token) {
    try {
        ...
        const data = await response.json();
        return { success: response.ok, message: data.message || "Deletion successful" };
    } catch (error) {
...
export async function filterDoctors(name, time, specialty) {
    // Ensuring null/empty parameters are formatted as 'null' strings or handled cleanly by the backend router
